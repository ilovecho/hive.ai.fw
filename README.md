...
> 배경 분석은 [ANALYSIS.md](ANALYSIS.md), 요구사항은 [REQUIREMENTS.md](REQUIREMENTS.md),
> 디바이스 등록·승인·모니터링·Claude 분석 에이전트 사용법은 [사용자메뉴얼.md](사용자메뉴얼.md),
> 개발 이어받기(규약·함정·검증 절차·시작 프롬프트)는 [HANDOFF.md](HANDOFF.md) 참고.
...
├── p01_sensor.py          # 서비스: 센서 시계열 저장/조회 + 디바이스 푸시 수신 [≒ s01_memo.php]
├── p01_device.py          # 서비스: 디바이스 등록/승인/차단/전송주기 제한
...
├── p03_ai_agent.py        # 데몬: Claude 분석 에이전트 (주기 분석 + 결과 저장)
├── d01_virtual_device.py  # 가상 IoT 디바이스 (온습도 측정·전송 데모)
├── w01_monitor.html       # 화면: 실시간 모니터링 + 디바이스 관리 (GET /monitor)
├── w01_agent.html         # 화면: 에이전트 관리 (GET /agent)
│
├── inc/
...
│
├── tools/
│   └── smoke_test.py      # 회귀 테스트: 임시 포트·임시 DB, 과금 없음 — 코드 수정 후 필수 실행
├── models/                # AI 모델 저장소 (pkl/tflite — 디렉터리=저장소 규약)
├── db/                    # SQLite hive_ai.db (자동 생성, WAL 모드)
└── etc/                   # systemd 유닛 샘플 (api / collector / agent)
```
...
| `register_device` | p01_device | 디바이스 자가 등록 (공개, state=승인대기) |
| `push_sensor` | p01_sensor | 디바이스 측정값 수신 (공개, device_key+승인+주기제한 검증) |
| `list_device` / `approve_device` / `block_device` / `config_device` | p01_device | 디바이스 관리 |
| `agent_status` / `agent_enable` / `agent_disable` / `agent_config` | p01_agent | Claude 에이전트 관리 |
| `run_agent_now` / `list_agent_result` | p01_agent | 즉시 분석 / 결과 조회 |
| (GET) `/health` | p00 | 무인증 헬스체크 |
| (GET) `/monitor`, `/agent` | p00 | 관리 화면 |

---

## 회귀 검증

코드를 수정했으면 실행한다 (임시 포트 8699·임시 DB 사용, 운영 DB·Claude 과금에 영향 없음):

```bash
python tools/smoke_test.py    # 17/17 통과가 정상
```
